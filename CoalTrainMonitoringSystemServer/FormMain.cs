...
namespace CoalTrainMonitoringSystemServer
{
    public partial class FormMain : Form
    {
        ...
        string serverIP = "192.168.5.111";
        string serverIP_rangeSensor = "192.168.5.111";
        //服务端的端口
        int serverPort = 6000;
        int serverPort_rangeSensor = 1000;

        public static Socket socketClient;
        public static Socket socketClient_rangeSensor;
        ...
        public FormMain()
        {
            ...
            label_Company.Top = Screen.PrimaryScreen.Bounds.Height - toolStrip1.Height - label_Company.Height - BORDER_WIDTH;
            label_Company.Left = Screen.PrimaryScreen.Bounds.Width - label_Company.Width;
            label_Company.BringToFront();
        ...
        void InitializeAllWindows()
        {
            ...
            try
            {
                ...
                threadListenLamp.Name = "threadListenLamp";
            ...
            try
            {
                Thread threadRangeSensorWatch = new Thread(new ThreadStart(ThreadRangeSensorWatch));
                threadRangeSensorWatch.Name = "threadRangeSensorWatch";
                threadRangeSensorWatch.Start();
            }
            catch
            {
                Globals.Log("1#测距传感器监听线程启动失败！");
                //MessageBox.Show("1#报警线程启动失败！");
            }

        }

        public void ThreadRangeSensorWatch()
        {
            //使用指定的地址族，套接字类型和协议实例化新的套接字
            socketWatch = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
            //将字符串地址转化为网际协议地址
            IPAddress ipAddress = IPAddress.Parse(serverIP_rangeSensor);//这里为服务器IP
            //使用指定地址和端口号实例化新的网络端点
            IPEndPoint endPoint = new IPEndPoint(ipAddress, serverPort_rangeSensor);//ipAddress为服务器IP ， 6000为服务器端口
            try
            {
                ...
                //启动线程
                threadWatch.Start();
                Globals.Log("服务器已经启动,开始监听客户端传来的信息!");
            }
            catch
            {
                Globals.Log("请检查测距服务器IP是否正确，或端口是否被占用？");
                socketWatch.Close();
                return;
            }

            socketClient_rangeSensor = socketWatch.Accept();
            string[] strs = socketClient_rangeSensor.RemoteEndPoint.ToString().Split(':');
            Globals.Log("测距传感器客户端IP：" + strs[0] + " 端口：" + strs[1]);
        ...
        private void WatchConnecting()
        {
            while (true)
            {
                ...
                {
                    Globals.Log("WatchConnecting" + ex.Message); //提示套接字监听异常
                    break;
                }
                Globals.Log("客户端连接成功,两端可以开始通信了...\r\n");
                //创建通信线程 
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (firstReceived > 0) //接受到的长度大于0 说明有信息或文件传来
                    {
                        ...
                        //int result = Convert.ToInt32(s, 16);
                        //float range = result * 0.0001F;
                        Globals.Log1(buffer.ToString());
                    }
                }
                catch (Exception ex)
                {
                    Globals.Log("ServerRecMsg" + ex.Message);
                    break;
                }
            }
        }
                ...
                try
                {
                    if (Globals.bSaveAlert_1)
                    {
                        ...
                        string folderNameThermal = Application.StartupPath + "\\SaveReport\\Thermal\\1\\"
                            + time.ToString("yyyy-MM-dd") + "\\" + time.ToString("HHmmss");
            ...
            try
            {
                ...
                if (drThermal.Exists && drCamera.Exists)
                {
                    ...
                    foreach (DirectoryInfo NextFolder in TheFolder.GetDirectories())
                    {
                        ...
                        fileName = fileName.Substring(0, 2) + ":" + fileName.Substring(2, 2) + ":" + fileName.Substring(4, 2);
            ...
            if (dev_num > 0)
            {
                try
                {
                    ...
                    if (display != null)
                    {
                        ...
                        {
                            _FormDisplayLst[DataDisplay.CurrSelectedWndIndex].Show();
                            Globals.Log("热成像相机连接成功");
                            DataDisplay.CurrSelectedWndIndex = display.GetDateDisplay().WndIndex;//更新选中框                  
                            display.GetDateDisplay().Play();
                        }
                        else
                        {
                            Globals.Log("热成像相机连接失败");
                        }
                    }
                }
                catch (Exception ex)
                {
                    Globals.Log("启动热成像相机" + ex.Message);
                }
            }
            else
            {

                Globals.Log("没有发现热成像相机");
            }
